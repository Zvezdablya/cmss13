...
/mob/living/simple_animal/hostile/retaliate/giant_lizard/moth/get_examine_text(mob/user)
	...
	if(stat == DEAD || user == src)
		desc = "Огромная моль."
		if(user == src)
			. += SPAN_NOTICE("\nОтдыхайте на земле чтобы восставливать 5% своего здоровья каждую секунду.")
			. += SPAN_NOTICE("Вы можете напрыгивать на цели с помощью [get_ability_mouse_name()].")
			. += SPAN_NOTICE("Вы будете агрессивно утаскивать и убивать своих жертв.")
	else if((user.faction in faction_group))
		desc = "[initial(desc)] Оно выглядит... дружелюбно?"
	else
		desc = initial(desc)
	if(isxeno(user))
		. += SPAN_DANGER("Это существо пугает вас и ваших сородичей одним лишь видом!")
	return .
